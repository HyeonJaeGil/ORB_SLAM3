...
public:
    ...
    // Input images: RGB (CV_8UC3) or grayscale (CV_8U). RGB is converted to grayscale.
    // Returns the camera pose (empty if tracking fails).
    Sophus::SE3f TrackMonocularMask(const cv::Mat &im, const cv::Mat &mask, const double &timestamp, const vector<IMU::Point>& vImuMeas = vector<IMU::Point>(), string filename="");
    ...
    void SaveKeyFrameTrajectoryTUM(const string &filename, Map* pMap);
    void SaveAllKeyFrameTrajectoryTUM(const string &filename);
