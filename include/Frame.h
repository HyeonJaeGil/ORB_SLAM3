...
public:
    ...
    Frame(const cv::Mat &imGray, const cv::Mat &imMask, const double &timeStamp, ORBextractor* extractor,ORBVocabulary* voc, GeometricCamera* pCamera, cv::Mat &distCoef, const float &bf, const float &thDepth, Frame* pPrevF = static_cast<Frame*>(NULL), const IMU::Calib &ImuCalib = IMU::Calib());
    ...
    // Extract ORB on the image with mask.
    void ExtractORBMasked(int flag, const cv::Mat &im, const cv::Mat &mask, const int x0, const int x1);
